          ...
          <div className="space-y-2">
            <div className="flex items-center space-x-2 text-indigo-600 font-semibold tracking-wide uppercase text-sm">
              ...
              {lecture.tags.map((tag) => (
                <span
                  key={tag}
                  className="text-slate-400 text-xs px-2 hidden sm:inline"
                >
                  • {tag}
                </span>
              ))}
        ...
        <main className="flex-1 min-w-0">
                ...
                <div className="bg-slate-50 px-6 py-4 border-b border-slate-200 flex items-center justify-between">
                  ...
                  >
                    ...
                    <span className="sm:hidden">
                      {showTranslation ? "Hide" : "VN"}
                    </span>
                  </button>
                </div>
                <div className="p-6 sm:p-8">
                  <div
                    className={`grid gap-8 ${showTranslation ? "md:grid-cols-2" : "grid-cols-1"}`}
                  >
                    {/* Original Text */}
                    ...
                    {showTranslation && (
                      <div className="flex flex-col pt-8 md:pt-0 border-t md:border-t-0 md:border-l border-slate-200 md:pl-8 animate-in fade-in slide-in-from-right-4 duration-300">
                        <h3 className="text-xs font-bold text-indigo-400 uppercase tracking-wider mb-4 border-b border-indigo-50 pb-2">
                          Vietnamese Translation
                        </h3>
                        <div
                          className="markdown-body prose prose-slate max-w-none prose-p:text-slate-600 prose-headings:text-slate-800"
                          dangerouslySetInnerHTML={renderMarkdown(
                            lecture.vietnameseMeaningPassage ||
                  ...
                  {lecture.keyTakeaways.map((point, index) => (
                    <li key={index} className="flex gap-4">
                      ...
                      <div className="bg-indigo-800/50 p-4 rounded-xl border border-indigo-700/50 flex-grow">
                        <div
                          className="text-indigo-100 leading-relaxed font-medium [&_strong]:text-white [&_strong]:font-extrabold [&_p]:m-0 [&_p]:inline-block"
          ...
          {activeTab === "exercises" && (
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <QuizSection
                standaloneQuestions={lecture.exercises}
                groupedQuestions={lecture.exerciseGroups}
              />
            </div>
          )}
        </main>
